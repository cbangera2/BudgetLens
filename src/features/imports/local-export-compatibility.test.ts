...
const describeLocalExport: typeof describe = localExportPath ? describe : describe.skip
const describeLocalTransactions: typeof describe = localTransactionExportPath
  ? describe
  : describe.skip

describeLocalExport("private local export compatibility", () => {
  it("accepts a real net-worth export without exposing its contents", async () => {
    if (!localExportPath) throw new Error("BUDGETLENS_REAL_EXPORT_PATH is required")
...
describeLocalTransactions("private local transaction export compatibility", () => {
  it("accepts a real transaction export without exposing its contents", async () => {
    if (!localTransactionExportPath) {
      throw new Error("BUDGETLENS_REAL_TRANSACTION_EXPORT_PATH is required")
    }
