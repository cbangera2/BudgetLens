...
// Cap: 1,800,000 bytes raw -- raised from 1,760,000 for the streaming +
// mappings wave (local re-measure 1,759,407 bytes on current main with 593
// bytes of headroom left; streaming adds ~5.2 KB, mappings fits as-is after
// rebase; ~25 KB forward headroom). History: 1,760,000 covered the tax-summary
// route; 1,750,000 the split + monthly-close wave; 1,705,000 proved too tight
// once receipt OCR landed. Still tight enough to bite on a real regression.
// NOTE: tripping this check every wave is now routine growth-accounting, not
// signal. If it fires a fourth time, invest instead in per-route code
// splitting (lazy dashboard/feature routes) to bend the curve.
...
const CAP_BYTES = 1_800_000 // raised deliberately; see note above
