...
- Package manager (verified 2026-09-06): generate the platform with `cap add ios --packagemanager CocoaPods`, not the SPM default. SPM builds fail under Xcode 26 CLI with unresolvable remote transitive deps (`KeychainSwift`, then `IONFilesystemLib` — each in turn), while CocoaPods builds clean with proper schemes. Revisit if Xcode/SPM heals; either way `ios/` stays gitignored.
