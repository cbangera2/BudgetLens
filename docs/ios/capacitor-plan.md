...
- Re-probe, same device (2026-09-05, scratch diagnostics v2, two launches with a reinstall between):
  - Upgrade retention PROVEN with an idempotent marker row (`upgrade-probe-1`): run 1 wrote it (total 145 = 144 demo + marker), reinstall-over preserved the container, run 2 read marker present with total still 145. A wipe would have lost the marker (the diag screen never reseeds demo data), so this is genuine retention, not reseed coincidence.
  - Assistant transports validated with EXACT production payloads and `apiKey: "test"`: `requestChatTurn` shape (system + history + tools + tool_choice auto + temperature + Bearer auth + abort signal) → HTTP 401; `sendToolResults` shape (assistant content + tool_calls + tool role) → HTTP 401. Both prove CORS + auth + payload acceptance; 401 is just the bogus key.
  - Live bug found by the probe: `provider.ts` used `AbortSignal.any()`, which does not exist on iOS 17 WebKit — every assistant request threw before hitting the network. Fixed with a manual composite signal (caller abort → AbortError, timeout → TimeoutError, timer cleared on settle) plus a regression test; verified by the passing re-probe above.
...
### Widget snapshot bridge + WidgetKit/Siri scaffold (2026-09-06, feat/widgets-siri — no device validation yet)

- Web bridge: `src/features/widget-bridge/` builds a SMALL versioned JSON payload (`version: 1`, net-worth latest + delta, month spend vs budget totals, top-5 categories, top-8 budget slices — pure derivation over `buildFinanceSnapshot`, no new aggregates/queries) and hands it to the `src/lib/native.ts` sink `writeWidgetSnapshot(json)` (append-only extension; existing signatures untouched).
- Sink ladder: App Group container is the real destination but unreachable from stock Filesystem JS, so native stages to `Documents/budgetlens-widget-snapshot.json` today (`via: "documents-staging"`) and the group-container mover lands in the device pass; web writes a `budgetlens.widget-snapshot` localStorage scratch for debugging; unconfigured group / storage failure is a graceful `{ok:false, via:"noop"}` that never throws.
- Refresh triggers are one-line `notifyImportCommitted / notifyRestoreCompleted / notifyTransactionsChanged / notifyBudgetsChanged` (+ coalescing `scheduleWidgetRefresh`) with exact wiring points documented in code — call-site wiring deliberately NOT landed here (mutation UIs are other agents' zones this round); writer tests cover every path incl. restore, version-bump rejection, and no-op fallbacks.
- Native scaffold (committed, NOT compiled here): `src/features/widget-bridge/native/` (`WidgetSnapshot.swift` version-gated reader, `BudgetLensWidget.swift` WidgetKit views, `SpendingIntent.swift` "how much did I spend on X" App Intent). `scripts/ios-patcher.mjs` (zero deps) copies/stamps them into the generated `ios/` project and applies the App Group entitlement, appex target registration (deterministic ids, marker-delimited idempotent pbxproj blocks), and Info.plist keys; `--dry-run`/`--check` + unit-tested pure parts allow verification without Xcode.
- App Group id: `group.com.cbangera2.budgetlens` (patcher default, overridable via `--app-group`). Widget bundle id: `<appId>.widget`. Deployment target for the appex: 17.0 (revisit when Spike 6 pins the iOS floor).
- Deferred device-validation pass (follow-up, needs real Xcode + device): `xcodebuild -list`/device build of the appex target, group-container read/write, Documents-staging mover, timeline reload policy, Siri dialog wording, largest-text/widget rendering, landing the one-line `notify*` wiring.
- Review corrections (2026-09-06, same branch): patcher now also wires host-app embedding (Embed App Extensions copy phase `dstSubfolderSpec 13` + target dependency/proxy into the host `App` target — without it the built appex never installs); widget timeline uses an hourly `.after` policy instead of `.atEnd`; widget struct carries `@main`; Siri intent honors its `month` parameter against the snapshot month; month spend sums the daily outflow series (unbudgeted expenses count) with goals contributing only the budget side.

### Home-screen quick actions (2026-09-06, feat/quick-actions — no device validation yet)

- Static items (patcher-written into the generated host `Info.plist` as `UIApplicationShortcutItems`, preserved/merged idempotently): `com.budgetlens.add` ("Add transaction", plus icon) and `com.budgetlens.budgets` ("View budgets", bookmark icon).
- Bridge mechanism: the injected SceneDelegate handler (marker-delimited extension block + one-line cold-start capture spliced into `scene(_:willConnectTo:options:)`, both idempotent) forwards the tapped type via `bridge.eval` dispatching `window.dispatchEvent(new CustomEvent("budgetlens-quick-action", { detail: <type> }))`. A plain CustomEvent keeps the web contract standard (unit tests dispatch the identical event); cold start stashes the type and flushes once the bridge WebView exists with one delayed retry, warm taps forward via `windowScene(_:performActionFor:)`.
- JS listener: `src/features/quick-actions/` maps `com.budgetlens.add` → `/transactions` (the page hosting the Add dialog; auto-opening the dialog needs a URL-param contract in the transactions feature — follow-up in that zone) and `com.budgetlens.budgets` → `/budgets` (paths verified against `src/app/router.tsx`, untouched); unknown types and malformed payloads are ignored. Mounting (one `setupQuickActionListener()` call at startup) is left for the app-shell owner to avoid cross-zone edits this round.
- Verified without Xcode: patcher pure/integration tests (plist entries, handler injection, byte-identical re-run via `--check`/tree-hash) + JS listener unit tests. Deferred device-validation pass (follow-up, needs real device): shortcut tap-through cold + warm, event timing against bundle boot/listener registration, `setupQuickActionListener()` mounting, largest-text rendering (no UI changes here, so nothing else).
