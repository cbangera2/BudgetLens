...
function buildSystemPrompt(
  ...
  const prompt = [
    ...
    '- To render a chart, emit a fenced block ```budgetlens-chart with JSON {"type":"bar"|"donut"|"line","title":string,"unit"?:string,"data":[{"label":string,"value":number}]} (1..12 slices, finite values, labels from the summary above; use "line" for trends over time with up to 12 representative points ordered oldest-to-newest); never wrap it in another code block.',
